...
const ALL_PROJECTS_DATA = [
  {
    ...
    imageUrl: "https://i.postimg.cc/vBQc4m99/img.png", // Already correct
    githubUrl: "#",
    liveUrl: "#",
  },
  {
    ...
    imageUrl: "https://i.postimg.cc/Vkm6G6Gh/Mobile-Hub-Logo.png", // Already correct
    githubUrl: "#",
    liveUrl: "#",
  },
  {
    ...
    imageUrl: "https://i.postimg.cc/htwSF1Kf/Pop-Balloon-logo.jpg", // Updated
    githubUrl: "#",
    liveUrl: "#",
  },
  {
    ...
    imageUrl: "https://i.postimg.cc/T3fhP9b1/Smart-Tag-Logo.png", // Updated
    githubUrl: "#",
    liveUrl: "#",
  },
  // "E-Shopper’s Hub – E-Commerce Management System" is removed
  {
    ...
    imageUrl: "https://i.postimg.cc/SsS89d8g/Admin-Panel-Logo.png", // Already correct
    githubUrl: "#",
    liveUrl: "#",
  },
  {
    ...
    imageUrl: "https://i.postimg.cc/RZn0ZDsc/Canon-Logo.jpg", // Updated
    githubUrl: "#",
    liveUrl: "#",
  },
  // Add other projects here if needed, but only 6 will be shown
];
...
const Portfolio: React.FC = () => {
  return (
    <section id="portfolio" className="w-full py-16 md:py-24 bg-background"> {/* Changed bg-neutral-DEFAULT to bg-background */}
      <div className="container mx-auto px-4 md:px-6">
        <h2 className="text-3xl font-bold tracking-tight text-center mb-4 text-foreground">My Projects</h2> {/* Changed text-neutral-text to text-foreground */}
        <p className="text-center text-muted-foreground mb-12">Here's a selection of projects I've worked on, showcasing my skills and experience.</p> {/* Changed text-neutral-muted to text-muted-foreground */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {DISPLAY_PROJECTS_DATA.map((project) => (
            <Card key={project.title} className="flex flex-col shadow-lg hover:shadow-xl transition-shadow duration-300 bg-card border-border overflow-hidden group"> {/* bg-card and border-border are good for theming */}
              ...
              <CardContent className="flex flex-col flex-grow p-6">
                <CardTitle className="text-xl font-semibold text-card-foreground mb-2">{project.title}</CardTitle> {/* text-card-foreground */}
                <CardDescription className="text-sm text-muted-foreground mb-4 flex-grow">{project.description}</CardDescription> {/* text-muted-foreground */}
                <div>
                  <p className="text-xs font-semibold text-card-foreground mb-2">Technologies:</p> {/* text-card-foreground */}
                  <div className="flex flex-wrap gap-2">
                    {project.technologies.map(tech => (
                      <span key={tech} className="text-xs bg-primary/10 text-primary px-2 py-1 rounded-full">{tech}</span> // Updated to use primary color
                    ))}
                  </div>
                </div>
              </CardContent>
              <CardFooter className="p-6 border-t border-border/50 space-x-2">
                {project.githubUrl !== "#" && (
                  <Button variant="outline" size="sm" asChild className="border-muted-foreground text-muted-foreground hover:border-primary hover:text-primary"> {/* Updated colors */}
